...
import needscroll.TutorialIsland.Task;
...
public class Action extends Task{
	...
	protected void use_component(int widget, int component, double time)
	{
		ctx.widgets.widget(widget).component(component).click(true);
		Condition.sleep((int) time);
	}
	...
	protected int item_amount(int id)
	{
		Item thing = ctx.inventory.select().id(id).poll();
		int amount = 0;
	...
	protected void deposit_all(int[] item_ids)
	{
		if (!ctx.bank.opened())
		{
			open_bank();
	...
	protected void rand_sleep(int time)
	{
		int base = (int) (time * .8);
		int var = (int) (time * .2);
		int rand1 = (int) (Math.random() * var * 2);
		int rand2 = (int) (Math.random() * var * 2);
		int variance = ((rand1 + rand2) / 2);
		...
		Condition.sleep(base + variance);
	}
	
	protected void rand_sleep(int base, int time2)
	{
		int rand1 = (int) (Math.random() * time2);
		int rand2 = (int) (Math.random() * time2);
		int variance = ((rand1 + rand2) / 2);
		int sign = (int) (Math.random() * 100);
